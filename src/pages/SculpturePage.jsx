...
export default function SculpturePage() {
    ...
    return (
      <main className="mx-auto max-w-xl px-4 py-24 text-center sm:px-6">
        ...
        <p className="mt-3 text-slate-300">
          ...
          <span className="text-white">{style.name}</span> pick. We'll create a 3D model preview
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-12 sm:px-6">
      <h1 className="text-3xl font-bold text-white">Get Yourself Sculpted in 3D</h1>
      ...
      <form onSubmit={handleSubmit} className="mt-10 space-y-10">
        {/* Step 1 — style */}
        <section>
          <h2 className="text-lg font-semibold text-white">1 · Pick a style</h2>
          <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3" role="radiogroup" aria-label="Sculpture style">
            {SCULPTURE_STYLES.map((s) => (
              ...
              >
                ...
                <p className="mt-2 text-sm font-semibold text-brand-400">from {formatINR(s.priceFrom)}</p>
              </button>
            ))}
          </div>
        </section>
        ...
        {/* Step 2 — photo */}
        <section>
          <h2 className="text-lg font-semibold text-white">2 · Add a photo</h2>
          <p className="mt-1 text-sm text-slate-500">
            A clear, well-lit photo works best. Front-facing for busts; full body for figurines.
          </p>
          <label
            htmlFor="photo"
            className="mt-4 flex cursor-pointer items-center justify-center gap-4 rounded-2xl border border-dashed border-white/20 bg-ink-900 px-4 py-8 text-sm text-slate-400 transition-colors hover:border-brand-500 hover:text-slate-300"
          >
            {photoPreview ? (
              <>
        ...
        <section className="space-y-4">
          <h2 className="text-lg font-semibold text-white">3 · Your details</h2>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              <label htmlFor="contact" className="mb-1.5 block text-sm font-medium text-slate-300">Phone, email, or Instagram</label>
              <input id="contact" required value={form.contact} onChange={set('contact')} className={inputClass} placeholder="9876543210 or @you" />
            </div>
          </div>
          <div>
            <label htmlFor="notes" className="mb-1.5 block text-sm font-medium text-slate-300">Anything we should know? (optional)</label>
